...
public class VorbisFile {
   ...
   public VorbisFile(String file) throws JOrbisException {
      ...
      VorbisFile.SeekableInputStream is = null;
   ...
   int fetch_headers(Info vi, Comment vc, int[] serialno, Page og_ptr) {
      ...
      } while(i >= 3 || this.get_next_page(og_ptr, 1L) >= 0);
         ...
         if (this.decode_ready) {
            ...
            if (result > 0) {
               ...
               if (this.vb.synthesis(op) == 0) {
                  int link = this.vd.synthesis_pcmout((float[][][])null, (int[])null);
                  this.vd.synthesis_blockin(this.vb);
                  this.samptrack += (float)(this.vd.synthesis_pcmout((float[][][])null, (int[])null) - link);
                  this.bittrack += (float)(op.bytes * 8);
                  if (granulepos != -1L && op.e_o_s == 0) {
                     link = this.seekable ? this.current_link : 0;
   ...
   public int bitrate(int i) {
      if (i >= this.links) {
         ...
         if (this.seekable) {
            return (int)Math.rint((double)((float)((this.offsets[i + 1] - this.dataoffsets[i]) * 8L) / this.time_total(i)));
         ...
         if (this.decode_ready) {
            ...
            if (samples != 0) {
               ...
               if (word == 1) {
                  ...
                  for(i = 0; i < samples; ++i) {
                     for(src = 0; src < channels; ++src) {
                        val = (int)((double)pcm[src][_index[src] + i] * 128.0D + 0.5D);
                        if (val > 127) {
                           val = 127;
                        } else if (val < -128) {
                           val = -128;
                  ...
                  if (host_endian == bigendianp) {
                     ...
                     if (sgned != 0) {
                        for(i = 0; i < channels; ++i) {
                           ...
                           for(j = 0; j < samples; ++j) {
                              val = (int)((double)pcm[i][src + j] * 32768.0D + 0.5D);
                              if (val > 32767) {
                                 val = 32767;
                              } else if (val < -32768) {
                                 val = -32768;
                        ...
                        for(i = 0; i < channels; ++i) {
                           ...
                           for(j = 0; j < samples; ++j) {
                              val = (int)((double)src[j] * 32768.0D + 0.5D);
                              if (val > 32767) {
                                 val = 32767;
                              } else if (val < -32768) {
                                 val = -32768;
                     ...
                     for(i = 0; i < samples; ++i) {
                        for(src = 0; src < channels; ++src) {
                           val = (int)((double)pcm[src][i] * 32768.0D + 0.5D);
                           if (val > 32767) {
                              val = 32767;
                           } else if (val < -32768) {
                              val = -32768;
                           ...
                           val += off;
                           buffer[var8++] = (byte)(val >>> 8);
                           buffer[var8++] = (byte)val;
                        }
                     }
                  } else {
                     for(i = 0; i < samples; ++i) {
